...
int  crtConvexIntersect( poly_sct *P, poly_sct *Q, poly_sct *R, int *r )
{
  ...
  do {
    ...
    SubVec( P->shp[a], P->shp[a1], A );
    SubVec( Q->shp[b], Q->shp[b1], B );
    cross = AreaSign( Origin, A, B );
    aHB   = AreaSign( Q->shp[b1], Q->shp[b], P->shp[a] );
    bHA   = AreaSign( P->shp[a1], P->shp[a], Q->shp[b] );

    /* If A & B intersect, update inflag. */
    code = SegSegInt( P->shp[a1], P->shp[a], Q->shp[b1], Q->shp[b], p, q );
    ...
    else if ( cross >= 0 )
    {
      if ( bHA > 0)
      {   //a = Advance( a, &aa, n, inflag == Pin, P[a] );
        if( inflag == Pin ) AddPoint(R->shp,r, P->shp[a]);
      ...
      {
        // b = Advance( b, &ba, m, inflag == Qin, Q[b] );
        if( inflag == Qin) AddPoint(R->shp,r, Q->shp[b]);
...
void AddPoint_1( tPolygond R, int *r, tPointd P)
{
